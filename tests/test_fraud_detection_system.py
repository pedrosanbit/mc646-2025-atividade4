...
def test_blacklisted_location(system):
    ...
    assert not result.is_fraudulent  # não necessariamente fraude, mas bloqueado
    assert result.is_blocked
...
def test_combined_high_risk_conditions(system):
    ...
    assert result.is_blocked
    assert result.verification_required
    assert result.risk_score == 100  # 50 + 30 + 20 acumulados
...
def test_blacklist_overrides_other_rules(system):
    ...
    assert result.is_blocked
    assert result.risk_score == 100  # sobrescreve qualquer soma
    assert result.is_fraudulent  # ainda verdadeiro por valor alto
    assert result.verification_required
